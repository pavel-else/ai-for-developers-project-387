...
function checkNoAdditionalPropertiesInternal(jsonObject, target, source, targetNode) {
    const diagnostics = [];
    for (const name of Object.keys(jsonObject)) {
        ...
        if (sourceProperty) {
            if (sourceProperty.type.kind === "Model") {
                ...
                const nestedDiagnostics = checkNoAdditionalPropertiesInternal(jsonObject[name], propertyNode?.value ?? target, sourceProperty.type, nestedTarget);
...
function getObjectLiteralNode(target) {
    return target !== undefined && "kind" in target && target.kind === SyntaxKind.ObjectLiteral
        ? target
        : undefined;
}
function getObjectLiteralProperty(node, name) {
    return node?.properties.find((property) => property.kind === SyntaxKind.ObjectLiteralProperty && property.id.sv === name);
}
