 ...
 */
function scanForErrorResponseSchemas(openapi, context) {
    if (!openapi.paths)
        return;
    for (const path of Object.values(openapi.paths)) {
        if (!path)
...
function scanPathForErrorResponseSchemas(path, context) {
    for (const method of methods) {
        const operation = path[method];
...
function scanOperationForErrorResponseSchemas(responses, context) {
